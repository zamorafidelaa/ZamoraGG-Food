...
const AdminMenus = () => {
  ...
  const openModal = (menu = null) => {
    if (menu) {
      ...
      setImagePreview(
        menu.imageUrl
          ? `${import.meta.env.VITE_API_BASE_URL}${menu.imageUrl}`
          : null
      );
      setImageFile(null);
    } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      ...
      {message && (
        <p className="mb-4 text-center text-green-600 font-semibold text-sm sm:text-base">
          {message}
        </p>
      )}
            ...
            {filtered.map((m) => (
              ...
              >
                ...
                <td className="px-3 sm:px-4 py-2">
                  {m.imageUrl ? (
                    <img
                      src={`${import.meta.env.VITE_API_BASE_URL}${m.imageUrl}`}
                      alt="Menu"
      ...
      <div className="md:hidden flex flex-col gap-4">
        ...
        {filtered.map((m) => (
          ...
          >
            ...
            <p>
              <span className="font-semibold">Price:</span> Rp {m.price}
            </p>
            <p>
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                <input
                  ...
                  className="border p-2 rounded focus:ring-2 focus:ring-blue-300 focus:outline-none text-sm sm:text-base"
                  required
                />
                <input
                  type="number"
                ...
                {imagePreview && (
                  <img
                    src={imagePreview}
                    alt="Preview"
                    className="w-32 h-32 object-cover mt-2 rounded mx-auto"
                  />
                )}
                <div className="flex flex-col sm:flex-row justify-end gap-2 mt-4">
                  <button
                    type="button"
                    onClick={closeModal}
                    className="px-3 sm:px-4 py-2 rounded bg-gray-200 hover:bg-gray-300 text-sm sm:text-base w-full sm:w-auto"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-3 sm:px-4 py-2 rounded bg-blue-500 text-white hover:bg-blue-600 text-sm sm:text-base w-full sm:w-auto"
                  >
                    {editId ? "Update" : "Add"}
                  </button>
                </div>
